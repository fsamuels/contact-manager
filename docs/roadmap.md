...
### Complete notes management in the Vue SPA
The SPA currently links to the JSP notes page for note management. This is
the last feature gap before the JSP UI can be decommissioned.

Tasks:
- Add a notes API client (`src/api/notes.ts`) — list, add, delete
- Add `NoteDto`, `NoteRequest` TypeScript types
- Add Vue Router routes: `/persons/:id/notes`
- Create `PersonNotesView` — note list, add-note form, delete (with confirmation or icon)
- Update `PersonListView` note icon to use `RouterLink` instead of a plain `<a>`
...
- Update `SpaController` to forward all `/app/**` paths to `index.html` (currently only covers `/app` and `/app/`; deep-linking to `/app/persons/new` directly will 404 from the server)

## Medium-term

### Remove the JSP layer
Once the SPA covers all features (person CRUD + notes), the JSP layer is
surplus. Removal steps:
...
- Protect all `/api/**` and JSP routes

### SPA deep-link fix (server-side)
`SpaController` currently only forwards `/app` and `/app/`. Any direct
navigation to `/app/persons/new` (e.g. browser refresh or shared link) will
return a 404 from Spring. The fix is to forward all requests under `/app/**`
that don't match a static resource to `/app/index.html`.
...
| Two UI layers (JSP + SPA) | Duplicated validation logic, CSS partially shared, navigation inconsistency | SPA notes completion |
